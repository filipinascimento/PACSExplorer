...
let k_DefaultParameters = {
	...
	selectedProperty : "Articles (%)",
	selectedFilterProperty : "Articles (%)",
	fromEnd: true,
	absolutePlot:false,
	smoothFilter : false,
...
function generateVisualization(data, inputParameters) {
	...
	var yearAxis = d3.axisTop(axisScale).tickFormat(d3.format("d")).ticks((PACSMaxYear-PACSMinYear+1)/5);
	...
	let chartHeight = 300 - chartMargin.top - chartMargin.bottom;
	...
	let chartAxisBottom = d3.axisBottom(chartXScale).tickFormat(d3.format("d")).ticks((PACSMaxYear-PACSMinYear+1)/5);;
	...
	displayedPACS.forEach(([pacsCode, level], index) => {
		...
		var colorScale = d3.scaleLinear()
			.domain([parameters.absoluteIntensity?colorMin:0, parameters.absoluteIntensity?colorMax:maxValue])
		...
		let chartDataset = featureData.map((d) => (
			{ "y": parameters.absolutePlot?d:d/maxValue})
		);
	...
	let histogramMargin = { top: 10, right: 20, bottom: 20, left: 50 };
		...
		let histogramHeight = 150 - histogramMargin.top - histogramMargin.bottom;
		...
		let histogramAxisBottom = d3.axisBottom(histogramXScale).tickFormat(d3.format("d")).ticks((PACSMaxYear-PACSMinYear+1)/5);;
		let histogramAxisLeft = d3.axisLeft(histogramYScale);
